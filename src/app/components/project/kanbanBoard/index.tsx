...
const KanbanBoard = ({ taskStatuses }: { taskStatuses: TaskStatus[] }) => {
  ...
  return (
    <div className="flex flex-col gap-2 h-[calc(100vh-125px)]">
      ...
      >
        <div className="flex flex-1 gap-3 items-stretch pb-2 w-[calc(100vw-300px)] overflow-x-auto customHorizontalScrollbar">
          {taskStatuses
            ?.sort(
          ...
          <div
            className="w-[40px] h-[40px] rounded-sm bg-neutral-100 border flex items-center justify-center cursor-pointer shrink-0"
            onClick={() => {
              queryClient.setQueryData(
                ["project-details", projectId],
                (old: ProjectDetails | undefined) => {
                  if (!old) return old;
                  return {
                    ...old,
                    taskStatuses: [...old.taskStatuses, old.taskStatuses[0]],
                  };
                }
              );
            }}
